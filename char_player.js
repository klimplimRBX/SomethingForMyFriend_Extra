...
class PlayerCharacter extends Character {
  ...
  draw(c) {
    ...
    // Quadrado — usa this.color (definido em startOnline para online, padrão roxo offline)
    const _bc = this.color || '#5B2D8E';
    c.shadowColor = _bc; c.shadowBlur = this.hitFlash > 0 ? 0 : 14;
    c.fillStyle   = this.hitFlash > 0 ? 'white' : _bc;
    c.strokeStyle = this.hitFlash > 0 ? _bc : 'rgba(255,255,255,0.55)';
    c.lineWidth = 3;
...
class PlayerCustomCharacter extends PlayerCharacter {
  ...
  _shoot(dt, other, projs) {
    // ── FIX 4: ONLINE PATCH — chars customizados remotos usam o patch da classe pai ──
    if (this._isRemote && !this._remoteMove) return;
    if (this._remoteMove) {
      return PlayerCharacter.prototype._shoot.call(this, dt, other, projs);
    }
    // ── fim patch ────────────────────────────────────────────────
  ...
  draw(c) {
    ...
    if (hasImg) {
      ...
      const _bc2 = this.color || '#5B2D8E';
      c.shadowColor=_bc2; c.shadowBlur=this.hitFlash>0?0:14;
      c.fillStyle=this.hitFlash>0?'white':_bc2;
      c.strokeStyle=this.hitFlash>0?_bc2:'rgba(255,255,255,0.55)'; c.lineWidth=3;
      c.fillRect(-sz/2,-sz/2,sz,sz); c.strokeRect(-sz/2,-sz/2,sz,sz);
      c.shadowBlur=0; c.fillStyle=this.hitFlash>0?_bc2:'rgba(255,255,255,0.7)';
      c.beginPath(); c.arc(0,0,sz*0.22,0,Math.PI*2); c.fill();
    }
    c.restore();
    if (this.freezeTimer>0) {
