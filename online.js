...
G._onlineUpdate = function(dt) {
  ...
  if (!ns.isHost && _receivedState) {
    ...
    _receivedState = null;
    return; // guest não roda simulação
  }
  ...
  // ── Envia inputs locais pro servidor (guest) ──────────────
  if (!ns.isHost) {
    NET.sendInput(dt, {
...
// Injeta um botão "Jogar Online" na tela de seleção do jogo.
// Chame esta função depois que o canvas for inicializado.
