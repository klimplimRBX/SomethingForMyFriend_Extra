...
const G = {
  ...
  _finaleActive:false, _finaleGuardT:0, _goShown:false,
  ...
  start() {
    ...
    if (window._menuUI) window._menuUI.hide();
    this._goShown = false;
    const make=(idx,x,y)=>{
  ...
  _drawMenu() {
    // HTML overlay handles the menu — skip canvas drawing
    if (window._menuUI && window._menuUI.active) return;
    this.btns={};
  ...
  _drawGameOver() {
    ...
    if (window._menuUI && !this._goShown) {
      this._goShown = true;
      window._menuUI.showGameOver(this.winnerText);
    }
    // HTML handles the display — skip canvas drawing
    if (window._menuUI) return;
    // ── fallback canvas gameover (no HTML) ──
    const cw=canvas.width/DPR, ch=canvas.height/DPR;
    ctx.save(); ctx.scale(DPR,DPR);
